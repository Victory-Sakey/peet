...
const MarkdownText: React.FC<{ text: string }> = ({ text }) => {
  ...
  const formatItalics = (str: string) => {
    ...
    return parts.map((part, index) => {
      if (part.startsWith("*") && part.endsWith("*")) {
        return (
          <em key={index} className="italic text-purple-700 not-italic font-medium">
            {part.slice(1, -1)}
          </em>
        );
      }
                ...
                if (numMatch) {
                  return (
                    <div key={lIdx} className="flex gap-3 items-start pl-1">
                      <span className="text-purple-600 font-extrabold text-xs sm:text-sm shrink-0 min-w-[20px] text-right mt-0.5">
                        {numMatch[1]}.
                      </span>
...
const RenderStyledDocument: React.FC<{ text: string; options: ResumeStyleOptions }> = ({ text, options }) => {
  ...
  const themeColors = {
    ...
    monochrome: { primary: "text-zinc-950", accent: "text-zinc-700", border: "border-zinc-200", line: "bg-zinc-100" }
  }[options.themeColor];
...
*Generated by PEET AI Career Suite (Offline Fallback)*`;
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
      ...
      <div className="bg-zinc-100/60 p-2 rounded-2xl border border-purple-200/50 flex gap-2.5 justify-center md:justify-start max-w-fit mx-auto md:mx-0 shadow-lg">
        {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs sm:text-sm font-bold transition-all duration-300 cursor-pointer ${
                isActive
                  ...
                  : "text-zinc-600 hover:text-white hover:bg-zinc-100/60 border border-transparent"
              }`}
            >
      ...
      {subTab === "coach" && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          <div className="hidden lg:block lg:col-span-1 space-y-4">
            <div className="glass-panel rounded-3xl p-8 space-y-5 border border-purple-200/50 shadow-xl bg-zinc-100/50">
              <h3 className="font-extrabold text-sm text-zinc-900 tracking-wide">Recommended Topics</h3>
              ...
              <div className="space-y-3 pt-2">
                {[
                  ...
                  <button
                    ...
                    className="w-full text-left p-4 rounded-2xl bg-zinc-100/40 hover:bg-purple-500/10 text-xs text-purple-800 hover:text-purple-700 font-bold border border-purple-500/5 hover:border-purple-200 transition-all duration-300 leading-normal cursor-pointer"
                  >
                    {q}
                  </button>
                ))}
          ...
          {/* Main chat window */}
          <div className="lg:col-span-3 flex flex-col h-[650px] glass-panel rounded-3xl overflow-hidden shadow-2xl border border-purple-500/15 bg-zinc-100/50 relative">
            {/* Subtle radial background glow */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-purple-500/5 rounded-full blur-3xl pointer-events-none" />

            {/* Header */}
            <div className="p-5 border-b border-purple-200/50 flex justify-between items-center bg-purple-950/20 relative z-10">
            ...
            >
              {chatMessages.map((msg) => {
                ...
                return (
                  ...
                  >
                    {!isUser && (
                      <div className="w-9 h-9 rounded-xl bg-purple-955/40 flex items-center justify-center text-sm border border-purple-500/15 shrink-0 text-purple-600">
                    ...
                    <div
                      className={`max-w-[80%] rounded-2xl px-5 py-4 text-xs sm:text-sm leading-relaxed whitespace-pre-wrap ${
                        isUser
                          ? "bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-tr-none shadow-lg shadow-purple-500/15 border border-purple-200"
                          : "bg-zinc-100/60 rounded-tl-none border border-purple-200/50 text-zinc-250 shadow-sm"
                      }`}
                    >
            ...
            <div className="lg:hidden px-6 pb-3 pt-2 flex gap-2.5 overflow-x-auto scrollbar-none select-none relative z-10">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() => sendChatMessage(q)}
                  className="shrink-0 px-4 py-2 rounded-full bg-zinc-100/80 border border-purple-500/15 text-[11px] font-bold text-purple-800 hover:text-purple-700 hover:border-purple-500/30 transition-all duration-300 cursor-pointer"
                >
                  {q}
                </button>
              ))}
            ...
            {/* Chat Input form */}
            <form onSubmit={handleSendChat} className="p-5 border-t border-purple-200/50 flex gap-3 bg-zinc-100/60 relative z-10">
              <input
                type="text"
                placeholder="Ask about resume tips, CV enhancement, job match index improvement..."
                value={chatInput}
                onChange={(e) => setChatInput(e.target.value)}
                className="flex-1 px-5 py-3.5 bg-zinc-100/40 rounded-2xl border border-purple-200/50 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500/40 text-xs sm:text-sm text-zinc-800 placeholder-zinc-505 transition-all"
              />
              <button
                type="submit"
      ...
      {subTab === "cv" && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5 glass-panel rounded-3xl p-8 sm:p-10 space-y-6 border border-purple-200/50 shadow-xl bg-zinc-100/50">
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Full Name</label>
                <input
                  type="text"
                  value={cvName}
                  onChange={(e) => setCvName(e.target.value)}
                  className="w-full px-5 py-3.5 bg-zinc-100/40 rounded-xl border border-purple-200/50 text-xs sm:text-sm text-zinc-800 focus:outline-none focus:ring-2 focus:ring-purple-500/25 focus:border-purple-500/40 transition-all placeholder-zinc-650"
                />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Job Title</label>
                <input
                  type="text"
                  value={cvTitle}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Email Address</label>
                <input
                  type="email"
                  value={cvEmail}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Phone Number</label>
                <input
                  type="text"
                  value={cvPhone}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Education Institute</label>
                <input
                  type="text"
                  value={cvEducation}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Degree Title</label>
                <input
                  type="text"
                  value={cvDegree}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Professional Biography</label>
                <textarea
                  rows={5}
                  value={cvIntro}
                  onChange={(e) => setCvIntro(e.target.value)}
                  className="w-full px-5 py-3.5 bg-zinc-100/40 rounded-xl border border-purple-200/50 text-xs sm:text-sm text-zinc-800 focus:outline-none focus:ring-2 focus:ring-purple-500/25 focus:border-purple-500/40 resize-none transition-all font-sans leading-relaxed"
                />
              </div>
          ...
          <div className="lg:col-span-7 glass-panel rounded-3xl p-8 sm:p-10 flex flex-col min-h-[550px] border border-purple-200/50 shadow-2xl bg-zinc-100/50">
            <div className="flex justify-between items-center mb-6 border-b border-purple-200/50 pb-4 flex-wrap gap-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-2 border-l border-zinc-200">
                  Document Styler
                </span>
              </div>
              ...
              {seekerCVContent && (
                <div className="flex items-center gap-3">
                  {/* View Mode Switcher */}
                  <div className="bg-zinc-100/80 p-1 rounded-xl border border-purple-200/50 flex gap-1 text-[10px] font-bold">
                    <button
                      onClick={() => setCvViewMode("document")}
                      className={`px-3 py-1 rounded-lg cursor-pointer transition-all ${
                        cvViewMode === "document" ? "bg-purple-650 text-white shadow-sm" : "text-zinc-600 hover:text-white"
                      }`}
                    >
                      Document
                    </button>
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg cursor-pointer transition-all ${
                        cvViewMode === "markdown" ? "bg-purple-650 text-white shadow-sm" : "text-zinc-600 hover:text-white"
                      }`}
                    >
                      Markdown
                    </button>
                  ...
                  <div className="flex gap-1.5">
                    <button
                      ...
                      className="p-2.5 text-zinc-600 hover:text-purple-700 bg-zinc-100/60 hover:bg-purple-500/10 rounded-xl border border-purple-200/50 hover:border-purple-500/30 transition-all duration-300 shadow-sm cursor-pointer"
            ...
            {seekerCVContent && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 mb-5 bg-zinc-100/60 rounded-2xl border border-purple-200/50 text-xs">
                {/* Color Theme Selector */}
                <div className="space-y-1.5">
                  <span className="font-bold text-zinc-600 block uppercase tracking-wider text-[9px]">Theme Color</span>
                  <div className="flex gap-2">
                    {[
                      ...
                      { id: "monochrome", bg: "bg-zinc-100 border border-zinc-700", label: "Mono" }
                    ].map((theme) => (
                      <button
                        key={theme.id}
                ...
                <div className="space-y-1.5 relative">
                  <span className="font-bold text-zinc-600 block uppercase tracking-wider text-[9px]">Font Style</span>
                  <button
                    type="button"
                    onClick={() => setIsCvFontOpen(!isCvFontOpen)}
                    className="w-full px-2.5 py-1.5 bg-zinc-100 rounded-lg border border-purple-200/50 text-zinc-600 flex items-center justify-between transition-all select-none text-left"
                  >
                    <span>
                  ...
                  {isCvFontOpen && (
                    <>
                      ...
                      <div className="absolute top-full left-0 right-0 mt-1 z-40 max-h-40 overflow-y-auto glass-panel rounded-lg p-1 border border-purple-200 shadow-xl scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
                          ...
                          <button
                            ...
                            className={`w-full text-left px-2 py-1.5 text-xs font-semibold rounded-md transition-colors ${
                              cvOptions.fontFamily === font
                                ...
                                : "text-zinc-500 hover:text-zinc-800 hover:bg-purple-500/10"
                            }`}
                          >
                            {font === "sans"
                              ? "Modern Sans (Inter)"
                ...
                <div className="space-y-1.5 relative">
                  <span className="font-bold text-zinc-600 block uppercase tracking-wider text-[9px]">Line Spacing</span>
                  <button
                    type="button"
                          ...
                          <button
                            ...
                            className={`w-full text-left px-2 py-1.5 text-xs font-semibold rounded-md transition-colors ${
                              cvOptions.spacing === space
                                ? "bg-purple-650 text-white"
                                : "text-zinc-455 hover:text-zinc-800 hover:bg-purple-500/10"
                            }`}
                          >
            ...
            {seekerCVContent ? (
              cvViewMode === "document" ? (
                ...
                <pre className="flex-1 p-6 bg-white/80 rounded-2xl border border-purple-200/50 text-xs sm:text-sm text-zinc-600 font-mono overflow-auto leading-relaxed max-h-[460px] scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
                  {seekerCVContent}
                </pre>
              )
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-10 text-zinc-500 space-y-4 border border-dashed border-purple-500/15 rounded-2xl bg-zinc-100/20">
                ...
                <div className="space-y-1 max-w-sm">
                  <h4 className="text-sm font-bold text-purple-800">No CV Drafted</h4>
                  <p className="text-xs text-zinc-500 leading-relaxed font-medium">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5 glass-panel rounded-3xl p-8 sm:p-10 space-y-6 border border-purple-200/50 shadow-xl bg-zinc-100/50">
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Target Position</label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Target Company</label>
                <input
                  type="text"
              ...
              <div className="space-y-2 relative">
                <label className="text-[10px] font-bold text-purple-600 block uppercase tracking-wider">Letter Tone</label>
                <button
                  type="button"
                  onClick={() => setIsLetterToneOpen(!isLetterToneOpen)}
                  className="w-full px-5 py-3.5 bg-zinc-100/40 rounded-xl border border-purple-200/50 text-xs sm:text-sm text-zinc-800 focus:outline-none focus:ring-2 focus:ring-purple-500/25 focus:border-purple-500/40 text-left flex items-center justify-between transition-all select-none cursor-pointer"
                >
                  <span>
                  ...
                  <ChevronDown className={`w-4 h-4 text-zinc-600 transition-transform ${isLetterToneOpen ? "rotate-180" : ""}`} />
                </button>
                {isLetterToneOpen && (
                  <>
                        ...
                        <button
                          ...
                          className={`w-full text-left px-3.5 py-2 text-xs font-semibold rounded-xl transition-colors ${
                            letterTone === tone.value
                              ? "bg-purple-650 text-white"
                              : "text-zinc-600 hover:text-zinc-800 hover:bg-purple-500/10"
                          }`}
                        >
                          {tone.label}
                        </button>
          ...
          <div className="lg:col-span-7 glass-panel rounded-3xl p-8 sm:p-10 flex flex-col min-h-[550px] border border-purple-200/50 shadow-2xl bg-zinc-100/50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="bg-zinc-100/80 p-1 rounded-xl border border-purple-200/50 flex gap-1 text-[10px] font-bold">
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg cursor-pointer transition-all ${
                        letterViewMode === "document" ? "bg-purple-650 text-white shadow-sm" : "text-zinc-600 hover:text-white"
                      }`}
                    >
                      Document
                    </button>
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg cursor-pointer transition-all ${
                        letterViewMode === "markdown" ? "bg-purple-650 text-white shadow-sm" : "text-zinc-600 hover:text-white"
                      }`}
                    >
                      Markdown
                    </button>
            ...
            {seekerCoverLetter ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-10 text-zinc-500 space-y-4 border border-dashed border-purple-500/15 rounded-2xl bg-zinc-100/20">
                ...
                <div className="space-y-1 max-w-sm">
                  <h4 className="text-sm font-bold text-purple-800">No Cover Letter Drafted</h4>
                  <p className="text-xs text-zinc-500 leading-relaxed font-medium">
