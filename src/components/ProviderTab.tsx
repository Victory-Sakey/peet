...
export const ProviderTab: React.FC = () => {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto px-4 py-8">
      {/* Subtab navigation */}
      <div className="flex flex-wrap border-b border-purple-200/50 gap-2 pb-px justify-center md:justify-start">
        {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-3 border-b-2 text-xs sm:text-sm font-bold transition-all ${
                isActive
                  ...
                  : "border-transparent text-zinc-500 dark:text-zinc-600 hover:text-purple-600"
              }`}
            >
        ...
        <div className="space-y-8 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="glass-panel rounded-3xl p-5 border border-purple-500/5 space-y-1 relative">
              <span className="text-zinc-600 dark:text-zinc-600 text-xs font-bold block">Active Listings</span>
              <span className="text-3xl font-extrabold text-gradient">{providerOpps.length}</span>
              <span className="text-[10px] text-zinc-500 dark:text-zinc-600 block">Open for applications & enrollment</span>
            </div>

            <div className="glass-panel rounded-3xl p-5 border border-purple-500/5 space-y-1">
              <span className="text-zinc-600 dark:text-zinc-600 text-xs font-bold block">Total Impressions</span>
              <span className="text-3xl font-extrabold text-zinc-700 dark:text-zinc-800">
                {totalViews.toLocaleString()}
              </span>
              <span className="text-[10px] text-zinc-500 dark:text-zinc-600 block flex items-center gap-0.5">
                <Eye className="w-3.5 h-3.5" /> views recorded
              </span>
            </div>

            <div className="glass-panel rounded-3xl p-5 border border-purple-500/5 space-y-1">
              <span className="text-zinc-600 dark:text-zinc-600 text-xs font-bold block">Applications</span>
              <span className="text-3xl font-extrabold text-zinc-700 dark:text-zinc-800">
                {applications.length}
              </span>
              <span className="text-[10px] text-zinc-500 dark:text-zinc-600 block flex items-center gap-0.5">
            ...
            <div className="glass-panel rounded-3xl p-5 border border-purple-500/5 space-y-1">
              <span className="text-zinc-600 dark:text-zinc-600 text-xs font-bold block">Conversion CTR</span>
              <span className="text-3xl font-extrabold text-zinc-700 dark:text-zinc-800">
                {clickThroughRate}%
              </span>
              <span className="text-[10px] text-zinc-500 dark:text-zinc-600 block">Views to clicks conversion</span>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-3xl p-6 space-y-4">
            <div className="flex justify-between items-center flex-wrap gap-4">
              <div>
                ...
                <p className="text-xs text-zinc-600 dark:text-zinc-600">Post new opportunities or audit active listing page views.</p>
              </div>
              <button
                onClick={() => setIsPostOpportunityOpen(true)}
                className="flex items-center gap-1.5 px-4 py-2.5 bg-purple-600 text-white rounded-xl text-xs font-bold transition-colors shadow-md shadow-purple-500/20"
              >
            ...
            <div className="space-y-3 pt-2">
              {providerOpps.map((opp) => (
                <div
                  key={opp.id}
                  className="p-4 bg-zinc-50/50 dark:bg-zinc-100 dark:bg-zinc-100/40 rounded-2xl border border-purple-500/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 group"
                >
                  <div className="flex gap-3 items-start flex-1">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shrink-0 ${
                      opp.type === "education"
                        ...
                        : "bg-purple-500/10 border-purple-200/50 text-purple-600"
                    }`}>
                      <OpportunityIcon logo={opp.logo} className="w-4 h-4" />
                    </div>
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h4 className="font-bold text-sm text-zinc-800 dark:text-zinc-800 group-hover:text-purple-600 dark:group-hover:text-purple-700 transition-colors">
                          {opp.title}
                        </h4>
                      ...
                      <div className="text-xs text-zinc-500 dark:text-zinc-600 dark:text-zinc-600 dark:text-zinc-600 font-medium">
                  ...
                  <div className="flex gap-6 items-center justify-between w-full md:w-auto border-t md:border-t-0 pt-2 md:pt-0 border-purple-500/5">
                    <div className="flex gap-4 text-xs font-semibold">
                      <div className="text-zinc-600 dark:text-zinc-600">
                        <span className="font-bold text-zinc-700 dark:text-zinc-700 dark:text-purple-800">
                    ...
                    <button
                      ...
                      className="p-2 text-zinc-600 dark:text-zinc-600 hover:text-red-500 rounded-xl hover:bg-red-500/5 transition-all cursor-pointer"
      ...
      {provSubTab === "applicants" && (
        <div className="glass-panel rounded-3xl p-6 space-y-4 animate-fade-in">
          <div>
            ...
            <p className="text-xs text-zinc-600 dark:text-zinc-600">Review seekers and shortlist or offer.</p>
          </div>

          {applications.length === 0 ? (
            <div className="py-12 text-center text-xs text-zinc-600 dark:text-zinc-600 italic">
            ...
            <div className="space-y-4">
              {applications.map((app) => {
                ...
                return (
                  <div
                    key={app.id}
                    className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-100 dark:bg-zinc-100/40 border border-purple-500/5 flex flex-col space-y-4"
                  >
                    {/* Seeker Identity */}
                    <div className="flex justify-between items-start gap-4 flex-wrap">
                      <div className="space-y-1">
                        <div className="text-sm font-bold text-zinc-800 dark:text-zinc-800">
                          {app.seekerName}
                        </div>
                        <div className="text-xs text-purple-600 dark:text-purple-700 font-semibold">
                    ...
                    <div className="p-3 bg-zinc-100/50 dark:bg-zinc-100/50 rounded-xl space-y-2 text-xs border border-purple-500/5">
                      <div className="font-bold text-zinc-700 dark:text-zinc-700 dark:text-purple-800">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {app.seekerSkills.map((s) => (
                          <span
                            key={s}
                            className="text-[9px] font-semibold bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-700 dark:text-purple-800 px-2 py-0.5 rounded"
                          >
                            {s}
                          </span>
                        ))}
                      </div>
                      <div className="pt-2 border-t border-purple-500/5 text-[10px] text-zinc-600 dark:text-zinc-600">
                         CV Attachment: <span className="font-bold text-purple-600 dark:text-purple-700">{app.cvUrl}</span>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="glass-panel rounded-3xl p-6 space-y-5 animate-fade-in">
          <div className="flex justify-between items-center flex-wrap gap-4">
            <div>
              ...
              <p className="text-xs text-zinc-600 dark:text-zinc-600">Search the PEET talent pool by title or skills.</p>
            </div>
            {/* Search Input */}
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-zinc-50 dark:bg-zinc-100 dark:bg-zinc-100/60 rounded-xl border border-zinc-200 dark:border-purple-200/50 focus:outline-none focus:ring-1 focus:ring-purple-600 text-xs text-zinc-750 dark:text-zinc-250"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredSeekers.map((s, i) => (
              <div
                key={i}
                className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-100 dark:bg-zinc-100/40 border border-purple-500/5 space-y-3 hover:border-purple-200/50 transition-colors"
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-bold text-sm text-zinc-800 dark:text-zinc-800">{s.name}</h4>
                    <span className="text-[10px] font-bold text-purple-650 dark:text-purple-700 block">
                      {s.title}
                    </span>
                  </div>
                  <span className="text-[10px] font-bold bg-purple-600/10 text-purple-600 dark:text-purple-700 px-2 py-0.5 rounded-lg border border-purple-200/50">
                ...
                <div className="flex flex-wrap gap-1">
                  {s.skills.map((sk) => (
                    <span
                      key={sk}
                      className="text-[9px] font-semibold bg-white dark:bg-zinc-800 text-zinc-500 dark:text-zinc-600 dark:text-zinc-600 dark:text-zinc-600 px-2 py-0.5 rounded border border-purple-500/5"
                    >
                      {sk}
                    </span>
                  ))}
                ...
                <div className="flex justify-between items-center text-[10px] text-zinc-600 dark:text-zinc-600 pt-1 border-t border-purple-500/5">
      ...
      {provSubTab === "profile" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
          ...
          <form onSubmit={handleUpdateCompany} className="lg:col-span-2 glass-panel rounded-3xl p-5 space-y-4">
            <h3 className="font-bold text-sm text-zinc-700 dark:text-zinc-800">Company / Provider Settings</h3>
            <p className="text-xs text-zinc-600 dark:text-zinc-600">Manage institution information that displays on active opportunity posts.</p>

            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-zinc-600 dark:text-zinc-600 block mb-1">Company / Institution Name</label>
                <input
                  type="text"
                  required
                  value={companyName}
                  onChange={(e) => setCompanyName(e.target.value)}
                  className="w-full px-3 py-2 bg-zinc-50 dark:bg-zinc-100 dark:bg-zinc-100/60 rounded-xl border border-zinc-200 dark:border-purple-200/50 text-xs text-zinc-750 dark:text-zinc-800"
                />
              </div>

              <div>
                <label className="text-[10px] font-bold text-zinc-600 dark:text-zinc-600 block mb-1">Industry / Institution Focus</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  <label className="text-[10px] font-bold text-zinc-600 dark:text-zinc-600 block mb-1">Organization Size</label>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <ChevronDown className={`w-4 h-4 text-zinc-600 dark:text-zinc-600 transition-transform ${isSizeDropdownOpen ? "rotate-180" : ""}`} />
                  </button>
                  {isSizeDropdownOpen && (
                    <>
                ...
                <div>
                  <label className="text-[10px] font-bold text-zinc-600 dark:text-zinc-600 block mb-1">Website URL</label>
                  <input
                    type="url"
                    required
                    value={companySite}
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-purple-600 text-white rounded-xl text-xs font-bold transition-colors shadow-sm cursor-pointer"
              >
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-100 dark:bg-zinc-100/60 rounded-2xl border border-purple-500/5 space-y-3">
              <div className="flex gap-3 items-center">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-purple-650 dark:text-purple-700 font-bold">
                    {companyIndustry}
                  </div>
                </div>
              </div>

              <div className="text-[11px] text-zinc-600 dark:text-zinc-600 space-y-1 pt-1 border-t border-purple-500/5">
