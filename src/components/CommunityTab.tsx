    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="space-y-3">
          <div className="glass-panel rounded-3xl p-5 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="font-extrabold text-sm text-zinc-700 dark:text-zinc-800">Browse Topics</h3>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all ${
                      isActive
                        ? "bg-purple-600/10 text-purple-600 dark:text-purple-700 border border-purple-200/50"
                        : "text-zinc-600 hover:text-white hover:bg-zinc-100/40"
                    }`}
                  >
                    {topic}
                  </button>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="flex justify-between items-center px-1">
            <h2 className="text-base font-bold text-zinc-600">
          ...
          {filteredPosts.length === 0 ? (
            <div className="glass-panel rounded-3xl p-16 text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-zinc-500 dark:text-zinc-600 text-sm max-w-xs mx-auto">
                ...
                >
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-extrabold uppercase px-2 py-0.5 bg-purple-600/10 text-purple-600 dark:text-purple-700 border border-purple-200/50 rounded">
                        {post.topic}
                      </span>
                    ...
                    <h3 className="font-extrabold text-base text-zinc-800 dark:text-zinc-900 group-hover:text-purple-600 dark:group-hover:text-purple-700 transition-colors">
                      {post.title}
                    </h3>

                    <p className="text-sm text-zinc-550 dark:text-zinc-600 line-clamp-2 leading-relaxed">
                      {post.content}
                    </p>
                  </div>
      ...
      {showAddPostModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-xl rounded-3xl overflow-hidden shadow-2xl border border-purple-200 max-h-[90vh] flex flex-col animate-scale-up">
            {/* Header */}
            <div className="p-5 border-b border-purple-200/50 flex justify-between items-center">
              ...
              <button
                ...
                className="text-zinc-600 hover:text-zinc-700 dark:hover:text-zinc-800"
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleCreatePost} className="p-6 overflow-y-auto flex-1 space-y-4">
              <div>
                <label className="text-[10px] font-bold text-zinc-600 block mb-1">Post Title</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3.5 py-2.5 bg-zinc-50 dark:bg-zinc-100/60 rounded-xl border border-zinc-200 dark:border-purple-200/50 text-xs"
                />
              </div>

              <div className="space-y-2 relative">
                <label className="text-[10px] font-bold text-zinc-600 block mb-1">Select Topic</label>
                <button
                  type="button"
                  onClick={() => setIsTopicDropdownOpen(!isTopicDropdownOpen)}
                  className="w-full px-3.5 py-2.5 bg-zinc-50 dark:bg-zinc-100/60 rounded-xl border border-zinc-200 dark:border-purple-200/50 text-xs text-left flex items-center justify-between text-zinc-700 dark:text-purple-800 transition-all select-none cursor-pointer"
                >
                  <span>{newTopic}</span>
                  <ChevronDown className={`w-4 h-4 text-zinc-600 transition-transform ${isTopicDropdownOpen ? "rotate-180" : ""}`} />
                </button>
                {isTopicDropdownOpen && (
                  <>
              ...
              <div>
                <label className="text-[10px] font-bold text-zinc-600 block mb-1">Content Body</label>
                <textarea
                  rows={6}
                  required
                  ...
                  className="w-full px-3.5 py-2.5 bg-zinc-50 dark:bg-zinc-100/60 rounded-xl border border-zinc-200 dark:border-purple-200/50 text-xs resize-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowAddPostModal(false)}
                  className="px-4 py-2 border border-zinc-200 dark:border-zinc-200 text-xs font-bold rounded-xl text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-800"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl border border-purple-200 max-h-[90vh] flex flex-col animate-scale-up">
            ...
            <div className="p-6 border-b border-purple-200/50 flex justify-between items-start gap-4">
              <div className="space-y-1 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-zinc-600">by {selectedPost.author}</span>
                </div>
              ...
              <button
                ...
                className="text-zinc-600 hover:text-zinc-700 dark:hover:text-zinc-800 p-1.5 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-200"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              ...
              {/* Core Post Content */}
              <div className="p-4 rounded-2xl bg-purple-600/5 border border-purple-200/50 leading-relaxed text-sm text-zinc-700 dark:text-purple-800 whitespace-pre-line">
                ...
                <div className="space-y-3">
                  {selectedPost.comments.length === 0 ? (
                    ...
                    selectedPost.comments.map((comment, i) => (
                      <div
                        key={i}
                        className="p-4 rounded-xl bg-zinc-50/50 dark:bg-zinc-100/40 border border-purple-500/5 space-y-1.5"
                      >
                        <div className="flex justify-between text-[10px] text-zinc-500 font-bold">
                          <span>{comment.author}</span>
                          <span>{comment.date}</span>
                        </div>
                        <p className="text-xs text-zinc-700 dark:text-purple-800 leading-relaxed">
                          {comment.text}
                        </p>
                      </div>
                    ))
              ...
              {/* Post Comment Form */}
              <form onSubmit={handleAddCommentSubmit} className="pt-4 border-t border-purple-200/50 space-y-3">
                <div>
                  <label className="text-[10px] font-bold text-zinc-600 block mb-1">Write a Reply</label>
                  <textarea
                    rows={3}
                    required
                    ...
                    className="w-full px-3.5 py-2 bg-zinc-50 dark:bg-zinc-100/60 rounded-xl border border-zinc-200 dark:border-purple-200/50 text-xs focus:outline-none focus:ring-1 focus:ring-purple-600 resize-none font-sans"
                  />
                </div>
