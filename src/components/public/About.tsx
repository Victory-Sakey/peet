    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
        {[
          ...
          <div key={i} className="glass-panel rounded-2xl p-5 border border-purple-500/5 space-y-1">
            ...
            <div className="text-[10px] sm:text-xs text-zinc-400 font-bold uppercase tracking-wider">{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="font-extrabold text-xl text-zinc-800 dark:text-zinc-200">Values that Drive Us</h3>
          <p className="text-xs text-zinc-400">Our commitment to seekers and partners alike.</p>
        </div>
          ...
          {values.map((v, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="w-10 h-10 rounded-xl bg-purple-600/10 text-purple-600 dark:text-purple-300 flex items-center justify-center mb-4">
                  <Icon className="w-5 h-5" />
                </div>
                <h4 className="font-bold text-sm text-zinc-800 dark:text-zinc-200 mb-2">{v.title}</h4>
