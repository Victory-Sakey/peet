  ...
  return (
    <div className="space-y-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 animate-fade-in">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3">
        ...
        {/* Billing Switcher */}
        <div className="inline-flex bg-zinc-950 p-1 rounded-xl border border-zinc-900 mt-4">
          <button
            onClick={() => setBilling("monthly")}
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
              billing === "monthly"
                ? "bg-zinc-900 text-purple-400 shadow-sm"
          ...
          return (
            <div
              ...
              className={`glass-panel rounded-2xl p-5 border relative flex flex-col justify-between ${
                p.popular
                  ? "border-purple-650 shadow-xl shadow-purple-550/5 lg:scale-102 z-10"
                  : "border-zinc-900"
              }`}
            >
              {p.popular && (
              ...
              <div className="space-y-5">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-500 pt-0.5 leading-relaxed">{p.desc}</p>
                  </div>
                ...
                <div>
                  {typeof p.price === "number" ? (
                    <div className="flex items-baseline">
                      ...
                      <span className="text-[10px] text-zinc-500 font-bold ml-1">/ month</span>
                    </div>
                  ) : (
                ...
                <ul className="space-y-2 pt-4 border-t border-zinc-900/60 text-xs">
                  {p.features.map((f, i) => (
              ...
              <button
                ...
                className={`w-full py-2.5 rounded-xl text-xs font-bold mt-6 transition-colors ${
                  p.popular
                    ...
                    : "bg-purple-600/10 hover:bg-purple-650/25 text-purple-400 border border-purple-500/15"
                }`}
              >
                {p.cta}
              </button>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-zinc-900">
                ...
                <th className="py-2.5 font-extrabold text-zinc-300">Starter</th>
                <th className="py-2.5 font-extrabold text-zinc-300">Growth</th>
                <th className="py-2.5 font-extrabold text-zinc-300">Enterprise</th>
              </tr>
            </thead>
            <tbody>
              {comparisonRows.map((row, i) => (
                <tr key={i} className="border-b border-zinc-900/40 hover:bg-zinc-950 transition-colors">
                  <td className="py-3 font-bold text-zinc-400">{row.name}</td>
                  <td className="py-3 text-zinc-500">
                    {row.starter === "✓" ? <Check className="w-3.5 h-3.5 text-purple-400" /> : row.starter === "✕" ? <X className="w-3.5 h-3.5 text-zinc-650" /> : row.starter}
                  </td>
                  <td className="py-3 text-zinc-500">
                    {row.growth === "✓" ? <Check className="w-3.5 h-3.5 text-purple-400" /> : row.growth === "✕" ? <X className="w-3.5 h-3.5 text-zinc-650" /> : row.growth}
                  </td>
                  <td className="py-3 text-purple-400 font-bold">
                    {row.enterprise === "✓" ? <Check className="w-3.5 h-3.5 text-purple-400" /> : row.enterprise}
                  </td>
                </tr>
              ))}
            </tbody>
